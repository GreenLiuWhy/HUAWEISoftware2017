package realcode.ver8;
/*
 * 改进之处：对函数进行了程序技术上的优化
 *changeSeverMethod()
 *初级：addS=0.18  detS=0.5
 *中级：iter=300 addS=0.1 detS=0.5 detS=0.72
 *高级：addS=0.1 detS=0.8 针对高级用例的前25次，删除的时候一次删除2个
 * */
import java.util.*;
...
public class Deploy
{
    ...
    public static String[] deployServer(String[] graphContent)
    {
        /**do your work here**/
        Deploy d=new Deploy(); 
        timer.schedule(new MyTask(), 88500);
    ...
    public HashMap dofire(ArrayList SP1)
    {
    	...
    	for(;!stop && iter<opiter;iter++)
    	{
    		...
    		nowSP=this.changeServerMethod(preSP,preNeed,iter);
...
//		System.out.println("----------------------------------------");
    ...
    public ArrayList changeServerMethod(ArrayList SP1,boolean need,int iter)
    {
    	...
    	if(SP.size()>1 && meths>=addS && meths<detS)
    	{
    		...
    		SP.remove(del);
    		if(this.nodeNum>=700 && iter<=25)//高级用例，前20次一次删除2个节点
    		{
