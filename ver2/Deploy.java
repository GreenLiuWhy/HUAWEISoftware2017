...
public class Deploy
{
    ...
    public static String[] deployServer(String[] graphContent)
    {
        /**do your work here**/
        Deploy d=new Deploy(); 
        timer.schedule(new MyTask(), 84000);
        d.getBasicInformation(graphContent);  
        //d.printRoudArray(d.getRealRoud(d.dofire(d.getIntaSeverPosition())));//可以正确输出了~~~
        //d.printTotleResult(d.changeForm(d.getRealRoud(d.dofire(d.getIntaSeverPosition()))));///可以了~~
    ...
    public HashMap dofire(ArrayList SP1)
    {
    	...
    	int T0=1000,opiter=10;//模拟退火的初试温度,模拟退火外循环的迭代次数
    	double T;//模拟退火的当前温度
...
//      		System.out.println("dofire print roud:"+Road.size());
//    		this.printRoud(Road,minSP);
//    		System.out.println("dofire print preroud:"+preRoud.size());
//    		this.printRoud(preRoud,preSP);
    ...
    public String[] changeForm(HashMap Roud)
    {
    	...
    	{
    		ArrayList<Integer> everyRoud=(ArrayList<Integer>)Roud.get(i);
    ...
    //测试用clone方法3：直接用system.arraycopy
    
    //////////////////////可以删除掉的调试用例//////////////////////////////
    public void printRoud(HashMap r,ArrayList SP)
    {
    	eee=this.deepCloneBaseFor(edge);//因为每次用的都是不同的，这样就不用每次都建了
    	this.buldSuperS(SP);
    	...
    	{
    		System.out.println("The "+i+" th rouds: ");
    		ArrayList<Integer> s=(ArrayList<Integer>)r.get(i);
    		for(int j=0;j<s.size()-1;j++)
    		{
    			int e=s.get(j);
    			int u=this.eee.get(e).u;
    			int v=this.eee.get(e).v;
    			System.out.print("idex="+e+"("+u+">"+v+")--->");
    		}
    		System.out.println();
    	}
    }
    ...
    //打印最终要回送出的结果
    public void printTotleResult(String[] ss)
    {
    	System.out.println("----------printTotleResult  begain:---------");
    	for(int i=0;i<ss.length;i++)
    		System.out.println(ss[i]);
    	System.out.println("--------printTotleResult  over-------");
    }
    
    //打印最终要转化形式的roud数组
    public void printRoudArray(HashMap<Integer,ArrayList> r)
    {
    	System.out.println("---------printRoudArray-----------");
    	for(int i=0;i<r.size();i++)
    	{
    		ArrayList a=r.get(i);
