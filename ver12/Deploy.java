...
public class Deploy
{
	...
	public static String[] deployServer(String[] graphContent)
    {
        ...
        return d.changeForm(d.getRealRoud(d.dofire(d.getRandomPop())));
        //d.dofire(d.getIntaSeverPosition());
        //return d.changeForm(d.getRealRoud());
    }
    //定时器
    	...
    	if(SP.size()>1 && meths>=addS && meths<detS)
    	{
    		...
    		if(this.nodeNum<700 || this.detIter>4)
    		{
    			...
    			}else
    			{
		    		...
		    		if(this.nodeNum>700 && iter<25)//如果是高级用例，则再减一个
		    		{
...
//    		}
    		...
    		while(true)//当没有加入成功
    		{
    			int addbor=rr.nextInt(SP.size());//在除超级源点和超级汇点的网络节点里随机选择一个加入
