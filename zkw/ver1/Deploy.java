...
public class Deploy
{
	...
	private int Sb,Se,minCost;//超级源点建立的起始边代号，超级源点建立的终止边代号
    ...
    public HashMap dofire(ArrayList SP1)
    {
    	...
    	System.out.println("zkw之前");
    	preCost=this.zkw()+SP1.size()*this.ServerMoney;//得到第初试服务器放置的最大流情况下的花费
    	System.out.println("zkw之后");
    	...
    	for(;!stop && iter<opiter;iter++)
    	{
    		...
    		nowCost=this.zkw()+nowSP.size()*this.ServerMoney;
	...
	public int dfs(int loc,int low)
	{
	    ...
	    int w,used=0;
	    for (int i=thead[loc]; i!=-1;i=eee.get(i).next)
	    {
	    	int tnode=eee.get(i).v;
	        if (Dis[tnode]==Dis[loc]-eee.get(i).cap && eee.get(i).cap>0 && !vis[tnode])
	        {
	        	int thes=0;
	        	if(low-used<eee.get(i).cap)
	        		thes=low-used;
	        	else
	        		thes=eee.get(i).cap;
                w=dfs(tnode,thes);
                minCost+=w*eee.get(i).cap;
                eee.get(i).cap-=w;
                eee.get(i^1).v+=w;
                if (used==low) 
                	return low;
    ...
    public void initNet()
    {
    	...
    	vis=new boolean[this.nodeNum+2]; 
