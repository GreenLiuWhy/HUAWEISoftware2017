...
public class Deploy
{
    ...
    public static String[] deployServer(String[] graphContent)
    {
        /**do your work here**/
        Deploy d=new Deploy(); 
        timer.schedule(new MyTask(), 8000);
        d.getBasicInformation(graphContent); 
        //看看realRoud之前传过来的路径的flow对不对,至少都是正数，那应该是对的
//        HashMap h=d.dofire(d.getIntaSeverPosition());
//        System.out.println("----------------这是原来有负边的Roud---------");
//        for(int i=0;i<h.size();i++)
//        {
//        	ArrayList a=(ArrayList)h.get(i);
//        	//System.out.println("第"+i+"条路径的flow="+a.get(a.size()-1));
//        }
//        System.out.println("---------------------------------------------------");
    ...
    public HashMap getRealRoud(HashMap old)
    {
    	...
    	for(int i=0;i<len;i++)//这个循环，可以将路径上的负边全部删除完毕，并且将每条路径的流量更新成了正确的流量
    	{//这个地方不应该是 i<old.size 因为 old有remove操作，会导致它的长度越来越小，导致循环过早结束
    		ArrayList<Integer> sub=(ArrayList<Integer>)old.get(i);//得到第i条路径
    		if(sub==null)//可能因为含有负边所以删除了
    			continue;
    		int HN=-1;//记录此时负边出现的编号，-1表示没有出现
    		...
    		//System.out.println("getRealRoud before HN!=1 allE:"+allE.toString());
    		if(HN!=-1)//如果此时是负边，则要改正从第0条路径到第HN-1条路径的路径flow
    		{//只要注释起这段就没有流量超出警告和消费节点不够的警告，所有错误有更大可能出现在这里
    			for(int ai=0;ai<HN;ai++)//得到前0-HN-1的路径
    			{
    				ArrayList<Integer> asub=(ArrayList<Integer>)old.get(ai);
    				if(asub==null)//可能这条路径是负边，所以前面的步骤已经删除了
    					continue;
    				int minflow=Inf;
    				for(int aj=0;aj<asub.size()-1;aj++)//得到这条路径应该有的流量
    				{
    					//System.out.println(" getRealRoudallE="+allE.size()+"  asub="+asub.size()+"  sub.get(aj)="+sub.get(aj)+"("+aj+")"+"  allE.get(sub.get(aj))="+allE.get(sub.get(aj)));
    					if(minflow>allE.get(asub.get(aj)))//本来把asub 写成了  sub
    						minflow=allE.get(asub.get(aj));
    				}
    				asub.set(asub.size()-1,minflow);//将该路径的流量更新
    				
    				for(int aaj=0;aaj<asub.size()-1;aaj++)//减去allE中的目前的总flow
    				{//因为前HN条路径上一定全部是正向边，所以可以这样做
    					int f=allE.get(asub.get(aaj));
    					f=f-minflow;
    					allE.remove(asub.get(aaj));
    					allE.put(asub.get(aaj), f);
    				}
    			}
    			i=-1;//也就是再从头开始
    			old.remove(HN);//删除含负边的路径
    			//allE.clear();//因为含有负边的路径的正向边可能是在前面走的，而是在负边之后走的，所以这里clear是不对的
    		}
    	}
    	//存路径
    	for(int i=0;i<len;i++)
