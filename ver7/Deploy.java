...
/*
 * 改进之处：对函数进行了程序技术上的优化，具体优化如下
 * 
 public boolean spfa(int allNode)：
{
	改进1：将 subPath和Dis放到同一个循环里进行初始化
	改进2：因为队列的cntains(E)操作用for循环来实现，所以不如用一个visit数组实现
	哈哈，改进完1和2，性能就提升了好多哎！大概3~7倍
}
 * */
import java.util.*;
...
public class Deploy
{
	...
	//private int smallCost;//smallCostBigFunction()得到的minCost，通过smallCost将它传给dofire()
    ...
    public HashMap dofire(ArrayList SP1)
    {
    	...
    	//HashMap<Integer,ArrayList> t=this.smallCostBigFunction();//先执行函数，才会更新smallCost
    	//preCost=this.smallCost+SP1.size()*this.ServerMoney;
    	...
    	for(;!stop && iter<opiter;iter++)
    	{
    		...
    		//HashMap<Integer,ArrayList> tt=this.smallCostBigFunction();//先执行函数，才会更新smallCost
    		//nowCost=this.smallCost+nowSP.size()*this.ServerMoney;
