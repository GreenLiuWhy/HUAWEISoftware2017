...
public class Deploy
{
	...
	private int minNode,deteNode,minNode2;//记录删除的时候提供最小流量的节点，和最终删除的节点
    	...
    	if(SP.size()>1 && meths>=addS && meths<detS)
    	{
    		...
    		else
    		{
    			...
    			if(del==-1)//当前一步没替换换时，会出现这样的情况
    				del=rr.nextInt(SP.size());
    			//System.out.println("minNode="+minNode+"   del="+del);
    		}
    		SP.remove(del);
    		
    		if(this.nodeNum>=700 && iter<=25)//高级用例，前20次一次删除2个节点,则删除第2小的
    		{
...
//        		for(int i=Sb+1;i<=Se;i=i+2)
//        		{
//        			if(eee.get(i).cap<mins && eee.get(i).cap>min)
//        			{
//        				min=eee.get(i).cap;
//        				minNs=eee.get(i).v;//真实的源点应该是超级节点的尾节点
//        			}
//        		}
//        		int del2;
//        		if(minN==minNode && minNs==minNode2)//说明现在出现了删除该最小的就满足不了消费节点的需求的情况
//        			del=rr.nextInt(SP.size());
//        		else
//        		{
//        			this.minNode2=minNs;
//        			del=SP.indexOf(this.minNode2);
//        		}
    		}
    		return SP;
    	}
